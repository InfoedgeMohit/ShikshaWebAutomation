...
public class SA_Search_Mobile_Object {
	...
	public SA_Search_Mobile_Object(WebDriver driver, ExtentTest child) {
		this.driver = driver;
		this.child = child;
	...
	public void OpenSearchLayer() throws Exception {
		try {
			...
			per.clickOnWEJS(driver.findElement(FetchData.readOR("OpenSearchLayerMobile")));
	...
	public void TypeCourseText(String TypeText) throws Exception {
		try {
			if (per.isAvailable(per.byXpath_To_StringXpath(FetchData.readOR("TypeinSearchBox")))) {
				Variables.elementWait(driver, 10, FetchData.readOR("TypeinSearchBox"));
				driver.findElement(FetchData.readOR("TypeinSearchBox")).sendKeys(TypeText);
			}
		} catch (Exception e) {
			throw e; // // Do nothing
		}
	...
	public void selectLocationFromSuggestor(String Location) {
		try {
			...
			java.util.List<WebElement> allLocations = driver.findElements(By.xpath("//p[@class='src-cTitle']"));
	...
	public void SelectFeesLeft() throws Exception {
		try {
			if (per.isAvailable(".//*[@id='feeSlider']/span[1]")) {
	...
	public void SelectFeesRight() throws Exception {
		try {
			if (per.isAvailable(".//*[@id='feeSlider']/span[2]")) {
				Variables.elementWait(driver, 5, FetchData.readOR("SelectFees"));
				Thread.sleep(2000);
	...
	public void SelectedFees() throws Exception {
		try {
			if (per.isAvailable("(//span[@class='leftLabel'])[1]")) {
	...
	public void SelectExam(String exam) throws Exception {
		try {
			Variables.elementWait(driver, 5, FetchData.readOR("SelectExamMobile"));
			Thread.sleep(2000);
	...
	public boolean isExamSliderVisible() {
		if (per.isAvailable(".//*[@id='examScoreSlider']/span[1]"))
			return true;
		else
			return false;
	}
	...
	public void SelectExamLeft() throws Exception {
		try {
			Variables.elementWait(driver, 5, FetchData.readOR("ExamScoreMobileLeft"));
			Thread.sleep(2000);
	...
	public void SelectExamRight() throws Exception {
		try {
			Variables.elementWait(driver, 5, FetchData.readOR("ExamScoreMobileRight"));
	...
	public void HitSearch() throws Exception {
		try {
			Variables.elementWait(driver, 5, FetchData.readOR("HitSearchMobile"));
			per.clickOnWEJS(driver.findElement(FetchData.readOR("HitSearchMobile")));
